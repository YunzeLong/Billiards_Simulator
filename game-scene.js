...
//         model_transform_shadow = model_transform_shadow.times(Mat4.translation(-1 * center_x, -1 * this.game.balls[0].y, -1 * center_z));
    ...
    display(context, program_state) {
        ...
        // Draw shadow
//         let model_transform_shadow = Mat4.identity();
//         let ballTop = this.game.balls[0].y + BALL_RADIUS_FOR_DRAW;
//         let displacement = Math.sqrt(this.game.balls[0].x**2 + this.game.balls[0].z**2);
//         let length = ((ballTop * displacement)/LIGHT_Y) + displacement;
//         let ratio = length / displacement;
//         let max_x = this.game.balls[0].x * ratio;
//         let max_z = this.game.balls[0].z * ratio;
//         let center_x = 0.5 * (max_x - this.game.balls[0].x) + this.game.balls[0].x;
//         let center_z = 0.5 * (max_z - this.game.balls[0].z) + this.game.balls[0].z;
//         let size_factor_x = (length - displacement) * 0.5;
        
// //         model_transform_shadow = model_transform_shadow.times(Mat4.translation(this.game.balls[0].x, this.game.balls[0].y, this.game.balls[0].z));
//         model_transform_shadow = model_transform_shadow.times(Mat4.translation(center_x, this.game.balls[0].y, center_z));
//         model_transform_shadow = model_transform_shadow.times(Mat4.scale(size_factor_x, 0.1, BALL_RADIUS_FOR_DRAW));
//         this.shapes.shadow.draw(context, program_state, model_transform_shadow, this.materials.shadow);
        ...
        /*
        // Draw the frame
        let model_transform_desk_frame = model_transform.times(Mat4.scale(DESK_OUTER_LENGTH/2, 1,DESK_OUTER_WIDTH/2));
        model_transform_desk_frame = model_transform_desk_frame.times(Mat4.translation(0,-BALL_RADIUS-1.2,0));
        this.shapes.cube.draw(context, program_state, model_transform_desk_frame, this.materials.desk_frame);
        // Draw the desk
        let model_transform_desk = model_transform.times(Mat4.scale(DESK_INNER_LENGTH/2, 1,DESK_INNER_WIDTH/2))
        model_transform_desk = model_transform_desk.times(Mat4.translation(0,-BALL_RADIUS-1,0))
        this.shapes.cube.draw(context, program_state, model_transform_desk, this.materials.desk_mat);
        // Draw Diamonds
        let model_transform_diamond = model_transform;
        // Draw Pockets here.
        let model_transform_pocket = model_transform.times(Mat4.rotation(Math.PI/2,1,0,0));
        let model_transform_pocket_1 = model_transform_pocket.times(Mat4.translation(DESK_INNER_LENGTH/2, DESK_INNER_WIDTH/2,7.5)).times(Mat4.scale(POCKET_RADIUS+1,POCKET_RADIUS+1,POCKET_DEPTH));
        this.shapes.pocket.draw(context, program_state, model_transform_pocket_1, this.materials.pocket);
        let model_transform_pocket_2 = model_transform_pocket.times(Mat4.translation(-DESK_INNER_LENGTH/2, DESK_INNER_WIDTH/2,7.5)).times(Mat4.scale(POCKET_RADIUS+1,POCKET_RADIUS+1,POCKET_DEPTH));
        this.shapes.pocket.draw(context, program_state, model_transform_pocket_2, this.materials.pocket);
        let model_transform_pocket_3 = model_transform_pocket.times(Mat4.translation(DESK_INNER_LENGTH/2, -DESK_INNER_WIDTH/2,7.5)).times(Mat4.scale(POCKET_RADIUS+1,POCKET_RADIUS+1,POCKET_DEPTH));
        this.shapes.pocket.draw(context, program_state, model_transform_pocket_3, this.materials.pocket);
        let model_transform_pocket_4 = model_transform_pocket.times(Mat4.translation(-DESK_INNER_LENGTH/2, -DESK_INNER_WIDTH/2,7.5)).times(Mat4.scale(POCKET_RADIUS+1,POCKET_RADIUS+1,POCKET_DEPTH));
        this.shapes.pocket.draw(context, program_state, model_transform_pocket_4, this.materials.pocket);
        let model_transform_pocket_5 = model_transform_pocket.times(Mat4.translation(0, DESK_INNER_WIDTH/2,7.5)).times(Mat4.scale(POCKET_RADIUS+1,POCKET_RADIUS+1,POCKET_DEPTH));
        this.shapes.pocket.draw(context, program_state, model_transform_pocket_5, this.materials.pocket);
        let model_transform_pocket_6 = model_transform_pocket.times(Mat4.translation(0, -DESK_INNER_WIDTH/2,7.5)).times(Mat4.scale(POCKET_RADIUS+1,POCKET_RADIUS+1,POCKET_DEPTH));
        this.shapes.pocket.draw(context, program_state, model_transform_pocket_6, this.materials.pocket);
        */
        /*
        //try drawing pocket
        let model_transform_pocket = model_transform.times(Mat4.rotation(Math.PI/2,1,0,0));
        for (let i = 0; i < 6; i++) {
            let model_transform_pocket_k = model_transform_pocket.times(Mat4.translation(POCKET_LOCATION[i][0], POCKET_LOCATION[i][1], 7.5)).times(Mat4.scale(POCKET_RADIUS, POCKET_RADIUS, POCKET_DEPTH+100));
                if (t % 1 < 0.5)
            this.shapes.pocket.draw(context, program_state, model_transform_pocket_k, this.materials.pocket);
        }
        */
    }
}
